...
data_raw <- read.csv("../../data/2b_categoricalprojection_untimed/study-trials.csv")
...
data$response_num = ifelse(data$response == "yes", 1, 0)
```
...
#hm, there appear to be some empty responses
data = data %>% 
  filter(response != "")
data$response = as.factor(as.character(data$response))
contrasts(data$response) # no is ref level, so predicting yes/projection responses
table(data$response) # more yes than no

```

```{r mixed effects linear model, include = FALSE, warning = FALSE, echo=FALSE}
m.full <- glmer(response ~ carousal_mean*crelativeValence + catissue + cSurprisal + (1 | Word) + (1 | content) + (1|workerid), data = data, family="binomial")  
...
m.valar <- glmer(response ~ carousal_mean*crelativeValence + (1 | Word) + (1 | content) + (1|workerid), data = data, family="binomial")  
summary(m.valar) # 
