...
projectivity_summarize <- projectivity_participant %>%
  group_by(Word, valence_mean, valence_bin, arousal_mean, arousal_bin) %>%
...
projectivity_participant_summary <- projectivity_participant %>%
  group_by(Word, valence_bin, arousal_bin) %>%
  summarize(mean_residuals_abs = mean(residuals_abs), mean_acceptability_norm = mean(acceptability_norm, na.rm = TRUE)) ## mean_acceptability_norm for display only
