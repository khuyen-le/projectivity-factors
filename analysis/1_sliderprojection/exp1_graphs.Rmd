...
```{r}
ggplot(data = data_by_word, aes(x = crelativeValence, y = meanBertSurprisal, label = Word)) +
  geom_point(width = .3,height = .025) +
  geom_smooth(method = 'lm') + 
  geom_text_repel() +
...
```{r}
ggplot(data = data_by_word, aes(x = carousal_mean, y = meanBertSurprisal, label = Word)) +
  geom_point(width = .3,height = .025) +
  geom_text_repel() +
  geom_smooth(method = 'lm') +
