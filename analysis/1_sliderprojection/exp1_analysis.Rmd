...
data_at_issue <- read.csv("../../data/atissue_data-byitem.csv")
```
...
names(data_at_issue)[names(data_at_issue) == "cresponse_mean"] <- "catissueness"

nrow(data_at_issue) #1673
data_proj <- data %>%
...
m.proj.atissue.full <- lmer(mean_projection ~ carousal_mean*crelativeValence*catissueness + (1 | Word) + (1 + (carousal_mean*crelativeValence*catissueness) | content), data = total, REML = FALSE)  
