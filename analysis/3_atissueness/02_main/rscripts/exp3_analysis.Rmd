...
```{r aggregate over items and save to .csv to analyse effect of at-issueness on projectivity}
length(unique(data$Word)) #54
length(unique(data$content)) #36
table(data$Word,data$content)

data_by_item = data %>%
    group_by(Word, content) %>%
    summarise(response_mean = mean(response_num), cresponse_mean = mean(catissueResponse))
nrow(data_by_item) #1673???

length(unique(data_by_item$Word)) #54
length(unique(data_by_item$content)) #36
dim(table(data_by_item$Word,data_by_item$content))
write_csv(data_by_item,"../../../../data/atissue_data-byitem.csv")
```
