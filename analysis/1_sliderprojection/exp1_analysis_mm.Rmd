...
avg_bert = bert_score %>%
  group_by(Verb) %>%
  summarize(mean_bert_score = mean(BertScore))
View(avg_bert)
...
```{r average by word}
data_by_word <- data %>%
  group_by(Word, voice, valence_group, valence_bin, valence_bin_group, arousal_bin, valence_arousal_group, carousal_mean, crelativeValence,mean_atissue,Surprisal,mean_bert_score,broad_class) %>%
